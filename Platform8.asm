...
!Skip:		sec 
			...
			cmp #$a0 				// is it floor? If yes, then continue walking 
...
.break
			...
			and #STATE_WALK_LEFT	// stop walking state (swith off walk bit)
...
!Skip:		clc 					// move player left in accordance with walkspeed
			...
			beq !EdgeCheck+	 	    // if no, then check screen edges
									...
									// there is no floor beneath the player
			lda Player_State		// so make the state revision
			and #STATE_WALK_RIGHT	// stop walking state (swith off walk bit)
			ora #STATE_FALL			// and start fall state  (switch on fall state)
			sta Player_State
			lda #$00 				// reset the fall index
