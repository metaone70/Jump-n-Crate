...
//   $0810-$0e10 Program Listing
//   $1000-$1376 Sound Effects
//   $2000-$21ff Sprite data
...
setup_sprites:   
		...
		lda #[STATE_FACE_RIGHT + STATE_WALK_RIGHT]	// set initial direction as facing right
		sta Player_State        
		
		lda Idle_Timeout
		sta Idle_Time

		//jsr OpenBorders				// IRQ driven open top&bottom border routine
...
!:		jsr delayfast 					// insert a delay, since this loop is out of the main delay routine
		...
		inc Player_y 					// increment player y coordinate
		inc Player_y 					// increment player y coordinate
		inc Player_y 					// increment player y coordinate -> Fall by 4 pixels
...
!:		lda Player_y					// get the x sprite coordinate
		sec 
		sbc #29 						// subtract $1b for the top border (by trial and error)
		lsr  							// divide by 8 -> 1 character is 8 bits long
		lsr 
		lsr 
		sta SpriteCharY
		 	 							// store it to character y variable
		rts 
	}
...
AdjustSpriteYCoor: {
		...
		adc #29
		sta Player_y
		PrintByteScreen(Player_y,2,2,2)
		rts 
}
...
Step1Sound:       			//step 1 sound
        lda #0 				//clean sid 
        sta $d404
        lda #15 			// volume
        sta $d418
        lda #110 			// voice 1 frequency, lo-hi .byte 
        sta $d400
        lda #70
        sta $d401
        lda #128			// voice 1 pulse waveform width, lo-hi .byte
        sta $d402
        lda #0
        sta $d403
        lda #33 			// voice 1 adsr
        ...
        lda #10
        sta $d406
        lda #129 			// voice 1 waveform(s) pulse
        sta $d404
		rts

Step2Sound:				    //step 2 sound
        lda #0 				//clean sid
        sta $d404
        lda #15 			// volume
        sta $d418
        lda #70 			// voice 1 frequency, lo-hi .byte
        sta $d400
        lda #50
        sta $d401
        lda #128 			// voice 1 pulse waveform width, lo-hi .byte 
...
Irq:    inc $d019            // acknowledge the interrupt by clearing the VIC's interrupt flag
self:   ldx #0-0             // 249 or 27
        stx $d012            // set next rasterline where interrupt shall occur
        bmi res25            // branch if bit 7 set
...
res25:  ldx #$1B             // decimal 27
        stx $d011            // undo scroll on any other line
        stx self+1           // set rasterline where interrupt shall occur
...
Floor1:				.byte $a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$20,$20,$20,$20,$20
					.byte $20,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$a0,$20,$20,$20
